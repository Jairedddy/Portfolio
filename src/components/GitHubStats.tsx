...
const GitHubStats: React.FC<GitHubStatsProps> = ({
  ...
  return (
    <motion.section
      className="relative overflow-hidden rounded-3xl border border-neon-cyan/40 bg-gradient-to-br from-surface-dark/80 via-background/70 to-surface-darker/80 p-6 shadow-neon-cyan/30 backdrop-blur-xl"
      ...
      <div className="relative z-10 space-y-6">
        <div className="flex flex-wrap items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              className="inline-flex items-center gap-2 rounded-full border border-neon-cyan/40 px-4 py-2 font-cyber text-xs uppercase tracking-[0.2em] text-neon-cyan transition hover:-translate-y-0.5 hover:bg-neon-cyan/10 disabled:cursor-not-allowed disabled:opacity-50"
            >
              <RefreshCcw className="h-4 w-4" />
              Refresh
            </button>
        ...
        {stats && status === "success" && (
          <div className="space-y-8">
            {stats.rateLimited && (
              <div className="rounded-xl border border-yellow-500/30 bg-yellow-500/5 p-3 text-center text-xs font-semibold text-yellow-200">
            ...
            >
              {statConfig.map((stat, index) => (
                <div
                  key={stat.key}
                  className="group rounded-2xl border border-border/60 bg-surface-dark/80 p-4 shadow-lg shadow-black/20 transition hover:border-neon-cyan/40 hover:shadow-neon-cyan/30"
                >
                  <p className="text-xs font-cyber uppercase tracking-[0.3em] text-muted-foreground">
                    {stat.label}
                  </p>
                  <p className="mt-2 text-2xl font-orbitron font-bold text-glow-cyan">
                    <span
            ...
            <div className="grid gap-6 lg:grid-cols-3">
              <div className="rounded-2xl border border-border/60 bg-surface-dark/70 p-5 backdrop-blur">
                  ...
                  <div className="mt-4 overflow-x-auto pb-2">
                    <div className="min-w-[320px]">
                      <div className="flex gap-4">
                        <div className="flex flex-col items-end gap-6 pt-6 text-[0.65rem] font-semibold uppercase tracking-[0.4em] text-muted-foreground">
                          {dayLabels.map((label) => (
                            <span key={label}>{label}</span>
                          ))}
                        </div>
                        <div className="flex-1">
                          <div className="ml-1 flex gap-1 text-[0.6rem] font-semibold uppercase tracking-[0.4em] text-muted-foreground">
                            {recentContributionWeeks.map((_, index) => (
                              <span key={`month-${index}`} className="w-4 text-center">
                          ...
                          <div className="mt-2 flex gap-1">
                            {recentContributionWeeks.map((week, weekIndex) => (
                              <div key={`week-${weekIndex}`} className="flex w-4 flex-col gap-1">
                                {week.map((day) => (
                                  <span
                                    key={day.date}
                                    className={`h-3 w-3 rounded-sm transition ${contributionLevelClass[day.level] ?? "bg-border/40"}`}
                          ...
                          <div className="mt-3 flex flex-wrap items-center gap-3 text-[0.65rem] text-muted-foreground">
                            ...
                            <div className="flex gap-1">
                              {contributionLegend.map((level) => (
                                <span
                                  key={`legend-${level}`}
                                  className={`h-3 w-5 rounded-sm ${contributionLevelClass[level] ?? "bg-border/40"}`}
                                />
                              ))}
                            </div>
                            <span>More</span>
              ...
              <div className="lg:col-span-2 rounded-2xl border border-border/60 bg-surface-dark/70 p-5 backdrop-blur">
                <p className="font-cyber text-sm uppercase tracking-[0.3em] text-muted-foreground">
                  ...
                  <div className="mt-4 space-y-4">
                    {stats.topRepositories.map((repo) => (
                      <a
                        ...
                        target="_blank"
                        rel="noreferrer"
                        className="block rounded-xl border border-border/60 bg-background/60 p-4 transition hover:border-neon-cyan/40 hover:bg-background/80"
                      >
                        <div className="flex flex-wrap items-center justify-between gap-3">
                          <div className="space-y-1">
                            <p className="text-lg font-orbitron text-neon-cyan">
                              {repo.name}
                            </p>
                            {repo.description && (
                        ...
                        <div className="mt-3 flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
                          {repo.language && (
                            <span className="rounded-full border border-neon-cyan/30 px-3 py-1 text-[0.65rem] font-semibold uppercase tracking-[0.3em] text-neon-cyan">
                              {repo.language}
                            </span>
                          )}
                          <span className="font-cyber uppercase tracking-[0.3em]">
                            View repository
                          </span>
                        </div>
                      </a>
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 text-xs text-muted-foreground">
              ...
              <span className="font-cyber uppercase tracking-[0.3em] text-neon-cyan">
                @{stats.username}
              </span>
            </div>
          </div>
